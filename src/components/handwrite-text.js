 ...
 * Renders text with a beautiful cursive handwriting style
 * 
...
export class HandwriteText extends HTMLElement {
  ...
  #render() {
    this.shadowRoot.innerHTML = `
      ...
      <div class="handwrite-container">
        <span class="handwrite-text"></span>
      </div>
    `;
  }
  ...
  #updateText() {
    ...
    const textEl = this.shadowRoot.querySelector('.handwrite-text');
    
    if (!textEl) return;
    
    const shouldAnimate = this.hasAttribute('animate');
    
    if (shouldAnimate && text.length <= 50) {
      // Character-by-character animation for shorter texts
      textEl.innerHTML = text.split('').map((char, i) => {
        if (char === ' ') return ' ';
        const delay = i * 0.03; // 30ms per character
        return `<span class="char" style="--delay: ${delay}s">${char}</span>`;
      }).join('');
    } else {
    ...
    this.dispatchEvent(new CustomEvent('rendered', { detail: { text } }));
  }

  #updateColor() {
    const color = this.getAttribute('color');
    if (color) {
      this.style.setProperty('--handwrite-color', color);
    }
  }
...
customElements.define('handwrite-text', HandwriteText);
