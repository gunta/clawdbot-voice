...
function GpuWaveform({ host }) {
  ...
  useEffect(() => {
    // Wait for document to be fully loaded before WebGPU init
    // (WebGPU adapter can fail if requested too early)
    if (document.readyState === 'complete') {
      initWebGPU();
    } else {
      window.addEventListener('load', initWebGPU, { once: true });
    }

    return () => {
      window.removeEventListener('load', initWebGPU);
      cleanup();
    };
  }, []);
